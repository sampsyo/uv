...
impl Lock {
    ...
    pub fn from_toml(s: &str) -> Result<Lock, toml::de::Error> {
        ...
        if let Some(requires_python) = &lock.requires_python {
            ...
            for distribution in &mut lock.distributions {
                for dep in &mut distribution.dependencies {
                ...
                for dep in distribution.optional_dependencies.values_mut().flatten() {
                    if let Some(marker) = &mut dep.marker {
                        *marker = marker.clone().simplify_python_versions(
                            python_version.clone(),
                            python_full_version.clone(),
                ...
                for dep in distribution.dev_dependencies.values_mut().flatten() {
                    if let Some(marker) = &mut dep.marker {
                        *marker = marker.clone().simplify_python_versions(
                            python_version.clone(),
                            python_full_version.clone(),
